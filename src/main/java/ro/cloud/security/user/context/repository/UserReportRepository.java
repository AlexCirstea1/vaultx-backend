...
public interface UserReportRepository extends JpaRepository<UserReport, UUID> {
    ...
    @Query("SELECT ur FROM UserReport ur WHERE ur.reporter.id = :reporterId AND ur.reported.id = :reportedId AND ur.createdAt > :since")
    List<UserReport> findRecentReports(@Param("reporterId") UUID reporterId,
